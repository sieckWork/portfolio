...
const Gallery = (function () {
    ...
    function createPage(container) {
        ...
        const imgSlider = createElement('img', [
            { name: 'src', value: 'img/characters_0' + idImage + '.png' },
        ...
        for (let index = 0; index < NB_IMG; index++) {


            const item = createElement('div', [{ name: 'name', value: index }, { name: 'class', value: 'item' }]);
            const img = createElement('img', [{ name: 'src', value: 'img/characters_0' + index + '.png' }]);

            item.addEventListener('click', () => {
                idImage = index;
                sliderContainer.style.display = 'flex';
                imgSlider.src = 'img/characters_0' + index + '.png';
